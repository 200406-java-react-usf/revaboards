...
export function corsFilter(req: Request, resp: Response, next) {
    ...
    resp.header('Access-Control-Allow-Origin', `*`); // dev only, dont use in prod
    ...
    //If this request is an OPTION request (aka "pre-flight request") send it back with a s staus of 200
